...
target_pattern = r'val scrollState = rememberScrollState\(\)\s*Column\(\s*modifier = Modifier\s*\.fillMaxSize\(\)\s*\.verticalScroll\(scrollState\)\s*\.padding\(14\.dp\)\s*\)\s*\{'

replacement = """val scrollState = rememberScrollState()
    ...
    Column(
        ...
        Row(
            ...
            Surface(
                ...
                Box(contentAlignment = Alignment.Center, modifier = Modifier.padding(horizontal = 24.dp)) {
                    Text(if (isBn) "লাইভ সেটিংস" else "Live Settings", color = if (previewMode == "LIVE_EDIT") Color.White else themeColors.buttonEqualBg, fontWeight = FontWeight.Bold, fontSize = 13.sp)
                }
            }
        }
