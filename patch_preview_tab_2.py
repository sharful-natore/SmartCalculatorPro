...
target = """    onOpenPdfInAppViewer: () -> Unit,
    onSaveProfile: () -> Unit = {}
) {
...
replacement = """    onOpenPdfInAppViewer: () -> Unit,
    onSaveProfile: () -> Unit = {}
) {
    val scrollState = rememberScrollState()
    ...
    Column(
        ...
        Row(
            ...
            Surface(
                ...
                Box(contentAlignment = Alignment.Center, modifier = Modifier.padding(horizontal = 24.dp)) {
                    Text(if (isBn) "সেটিংস (Settings)" else "Settings (Margin/Padding)", color = if (previewMode == "LIVE_EDIT") Color.White else themeColors.buttonEqualBg, fontWeight = FontWeight.Bold, fontSize = 13.sp)
                }
            }
        }
