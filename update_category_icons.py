...
def update_file(filepath):
    ...
    # Pattern for the Category Icon Box
    icon_box_pattern = r'(\.clip\(CircleShape\)\s*\n\s*\.background\()Color\.White(\),.*?Icon\([^)]*?imageVector\s*=\s*category\.icon,.*?tint\s*=\s*)themeColors\.buttonEqualBg(.*?size\(16\.dp\)\s*\n\s*\)\s*\n\s*})'
    
    content = re.sub(icon_box_pattern, r'\g<1>themeColors.buttonEqualBg\g<2>Color.White\g<3>', content, flags=re.DOTALL)
